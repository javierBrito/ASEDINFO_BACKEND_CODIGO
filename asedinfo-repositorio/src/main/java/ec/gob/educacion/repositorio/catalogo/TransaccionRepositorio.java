...
public interface TransaccionRepositorio extends JpaRepository<Transaccion, Long> {
	...
	@Query(nativeQuery = false, value = "select r from Transaccion r where ((datediff(r.fechaFin, Now()) < 0 or datediff(fecha_fin, Now()) <= 5) or ((r.fechaCambia is not null) and datediff(r.fechaCambia, Now()) <= 0)) and r.estado = 'A'")
